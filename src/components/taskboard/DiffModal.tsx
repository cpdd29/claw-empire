...
function DiffModal({ taskId, onClose }: DiffModalProps) {
  ...
  useEffect(() => {
    Promise.allSettled([getTaskDiff(taskId), getTaskVerifyCommit(taskId)]).then(([diffResult, verifyResult]) => {
      if (diffResult.status === "fulfilled") {
        ...
        if (!d.ok)
          setError(d.error || t({ ko: "未知错误", en: "Unknown error", ja: "不明なエラー", zh: "未知错误" }));
        else setDiffData(d);
      } else {
      ...
      if (verifyResult.status === "fulfilled") {
        ...
        if (!v.ok)
          setVerifyError(v.error || t({ ko: "校验失败", en: "Verification failed", ja: "検証失敗", zh: "校验失败" }));
  ...
  const handleMerge = useCallback(async () => {
    ...
    try {
      ...
      setActionResult(
        result.ok
          ? `${t({ ko: "合并完成", en: "Merge completed", ja: "マージ完了", zh: "合并完成" })}: ${result.message}`
          : `${t({ ko: "合并失败", en: "Merge failed", ja: "マージ失敗", zh: "合并失败" })}: ${result.message}`,
      );
      if (result.ok) setTimeout(onClose, 1500);
    } catch (e: unknown) {
      setActionResult(
        `${t({ ko: "错误", en: "Error", ja: "エラー", zh: "错误" })}: ${e instanceof Error ? e.message : String(e)}`,
      );
    } finally {
      setMerging(false);
    }
    ...
    try {
      ...
      setActionResult(
        result.ok
          ...
          : `${t({ ko: "丢弃失败", en: "Discard failed", ja: "破棄失敗", zh: "丢弃失败" })}: ${result.message}`,
      );
      if (result.ok) setTimeout(onClose, 1500);
    } catch (e: unknown) {
      setActionResult(
  ...
  const verifyVerdictLabel = (() => {
    switch (verifyData?.verdict) {
      case "ok":
        return t({ ko: "通过", en: "Passed", ja: "成功", zh: "通过" });
      case "dirty_without_commit":
        return t({ ko: "未提交变更", en: "Uncommitted changes", ja: "未コミット変更", zh: "未提交变更" });
      case "commit_but_no_code":
        return t({ ko: "无代码变更", en: "No code changes", ja: "コード変更なし", zh: "无代码变更" });
      case "no_commit":
        return t({ ko: "无提交", en: "No commit", ja: "コミットなし", zh: "无提交" });
      case "no_worktree":
        return t({ ko: "无工作树", en: "No worktree", ja: "ワークツリーなし", zh: "无工作树" });
      default:
        return t({ ko: "未知", en: "Unknown", ja: "不明", zh: "未知" });
    }
  })();
        ...
        <div className="flex items-center justify-between border-b border-slate-700 px-5 py-3">
          <div className="flex items-center gap-2">
            <span className="text-lg font-bold text-white">
              {t({ ko: "Git 差异", en: "Git Diff", ja: "Git 差分", zh: "Git 差异" })}
            </span>
            {diffData?.branchName && (
          ...
          <div className="flex items-center gap-2">
            ...
            >
              {merging ? "..." : t({ ko: "合并", en: "Merge", ja: "マージ", zh: "合并" })}
            </button>
            <button
            ...
            >
              {discarding ? "..." : t({ ko: "丢弃", en: "Discard", ja: "破棄", zh: "丢弃" })}
            </button>
            <button
              onClick={onClose}
              className="rounded-lg p-1.5 text-slate-400 transition hover:bg-slate-800 hover:text-white"
              title={t({ ko: "关闭", en: "Close", ja: "閉じる", zh: "关闭" })}
            >
              X
            </button>
          </div>
        ...
        <div className="flex-1 overflow-auto p-5">
          {loading ? (
            ...
            <div className="flex items-center justify-center py-12 text-red-400">
              {t({ ko: "错误", en: "Error", ja: "エラー", zh: "错误" })}: {error}
            </div>
          ) : !diffData?.hasWorktree ? (
            <div className="flex items-center justify-center py-12 text-slate-500">
              {t({
                ko: "找不到该任务的 worktree（非 Git 项目或已合并）",
                ...
                <div className={`rounded-lg border p-3 ${verifyToneClass}`}>
                  ...
                  {verifyData.uncommittedFiles && verifyData.uncommittedFiles.length > 0 && (
                    <p className="mt-2 text-xs">
                      {t({
                        ko: `未提交变更 ${verifyData.uncommittedFiles.length} 项`,
              ...
              {diffData.stat && (
                <div>
                  <h3 className="mb-1 text-sm font-semibold text-slate-300">
                    {t({ ko: "摘要", en: "Summary", ja: "概要", zh: "摘要" })}
                  </h3>
