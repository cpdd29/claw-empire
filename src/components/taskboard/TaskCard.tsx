    ...
    >
      <div className="mb-2 flex items-start justify-between gap-2">
        ...
        <span
          ...
          title={`${t({ ko: "优先级", en: "Priority", ja: "優先度", zh: "优先级" })}: ${priorityLabel(task.priority, t)}`}
        >
      ...
      <div className="mb-3 flex flex-wrap items-center gap-1.5">
        ...
        {isHiddenTask && (
          <span className="rounded-full bg-cyan-900/60 px-2 py-0.5 text-xs text-cyan-200">
            🙈 {t({ ko: "隐藏", en: "Hidden", ja: "非表示", zh: "隐藏" })}
          </span>
        )}
        {department && (
        ...
        <div className="flex items-center gap-1.5">
          {assignedAgent && assignedLabel ? (
            ...
            <span className="text-xs text-slate-500">
              {t({ ko: "未分配", en: "Unassigned", ja: "未割り当て", zh: "未分配" })}
            </span>
          )}
        </div>
                  ...
                  <div key={subtask.id} className="flex items-center gap-1.5 text-xs">
                    ...
                    {subtask.delegated_task_id && subtask.status !== "done" && (
                      <span
                        ...
                        title={t({ ko: "已委派", en: "Delegated", ja: "委任済み", zh: "已委派" })}
                      >
                        🔗
                      </span>
                    )}
      ...
      <div className="flex flex-wrap items-center gap-1.5">
        {canRun && (
          <button
            ...
            title={t({ ko: "运行任务", en: "Run task", ja: "タスク実行", zh: "运行任务" })}
          ...
          >
            ▶ {t({ ko: "运行", en: "Run", ja: "実行", zh: "运行" })}
          </button>
        )}
        {canPause && (
          <button
            onClick={() => onPauseTask!(task.id)}
            title={t({ ko: "暂停任务", en: "Pause task", ja: "タスク一時停止", zh: "暂停任务" })}
          ...
          >
            ⏸ {t({ ko: "暂停", en: "Pause", ja: "一時停止", zh: "暂停" })}
          </button>
        )}
        {canStop && (
          <button
            ...
            title={t({ ko: "取消任务", en: "Cancel task", ja: "タスク停止", zh: "取消任务" })}
        ...
        {canResume && (
          <button
            ...
            title={t({ ko: "恢复任务", en: "Resume task", ja: "タスク再開", zh: "恢复任务" })}
        ...
        {canHideTask && !isHiddenTask && onHideTask && (
          ...
          >
            🙈 {t({ ko: "隐藏", en: "Hide", ja: "非表示", zh: "隐藏" })}
          </button>
        )}
        {canHideTask && !!isHiddenTask && onUnhideTask && (
          <button
            onClick={() => onUnhideTask(task.id)}
            title={t({ ko: "恢复隐藏任务", en: "Restore hidden task", ja: "非表示タスクを復元", zh: "恢复隐藏任务" })}
          ...
          >
            👁 {t({ ko: "恢复", en: "Restore", ja: "復元", zh: "恢复" })}
          </button>
        )}
        {canDelete && (
          <button
            ...
            title={t({ ko: "删除任务", en: "Delete task", ja: "タスク削除", zh: "删除任务" })}
